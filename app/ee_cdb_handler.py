...
class MainPage(webapp2.RequestHandler):
    ...
    def get(self):
        ...
        dsid = self.request.get('dsid', None)
        ...
        #Grab kml
        sql = "SELECT * FROM get_layer_as_kml('%s','%s','%s','%s','%s_%s',6)"  % (source, type, sciname, dsid, dsid, sciname.replace(' ', '_'))
        url = 'http://mol.cartodb.com/api/v2/sql?%s' % urllib.urlencode(dict(q=sql))
        value = urlfetch.fetch(url, deadline=60).content

        for row in value.rows:
            sql = "INSERT INTO %s (scientificname, dataset_id, seasonality, kml) VALUES('%s','%s','%s','%s')" % (ft_id, sciname, dsid, row.seasonality, row.kml)
        #url = ''
